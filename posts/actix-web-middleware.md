...
Actix does actually provide a [wrap_fn](https://docs.rs/actix-web/4.0.0-beta.4/actix_web/struct.Scope.html#method.wrap_fn) helper to make middleware with only a closure, similar to the JS example. This doesn't always work, so let's continue so we can see what's happening under the hood.
...
So we need a `Transform`, a `Service`, and maybe an extractor, but how do they fit together? For many middlewares, it’s most of the complexity turns out to be boilerplate, but it still helps to understand what that boilerplate is actually doing. First, let's take a step back and understand more generally what a `Service` represents.
...
A middleware, then, is a special case of a `Service` that calls another `Service` as part of its invocation. The closest parallel in the Javascript example above is calling the `next` function.
...
There's a single function, `new_transform`, that creates a new instance of the middleware `Service`. The `service` argument is the service that the new middleware should wrap. 

`new_transform` returns a `Future`, but in this example we can create a `Ready` future since there's no actual asynchronous work to be done. This is similar to using Javascript's `Promise.resolve` to place a value inside a Promise.
...
As discussed, the types mostly match those used for the middleware service type, and `new_transform` creates an instance of the middleware and wraps it in a future with `ready`.
...
As above, this function uses the extensions API to get the user information, and then returns it inside a `Ready` future. We also implement `Deref` to make it easy to use the embedded `AuthenticationInfo` object.
...
Now that we have our extractor, we can drop it into the parameter list of any handler.
