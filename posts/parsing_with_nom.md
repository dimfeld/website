...
Writing a parser seems like a difficult task if you haven't done it before. It conjures visions of obscure grammars that feed into arcane tools that generate thousands of lines of unreadable code.
...
Here's an example of using `nom` to parse Roam Research's markdown-like syntax.
...
Our first parser just gets a word. `take_while1(nonws_char)` creates a parser that reads at least one character matching `nonws_char`.
...
`markdown_link` is for external links. It uses the `pair` combinator which works similarly to `tuple` but for just two parsers next to each other. This returns a tuple consisting of the link title and the URL. `delimited` comes with `nom` and works similarly to our `fenced` parser, but `fenced` works better when the ending pattern is more than one character.
